...
# tmpenv[,myVars] <- (tmpenv[,myVars] - env_mu[myVars])/env_sc[myVars]
tmpenv$Year <- tmpenv$Year - eStartYr

subData <- tmpenv[,myVars]
subData[is.na(subData)] <- -10000

#Add years beyond the last year of observed environmental data if necessary
...
data <- list( yShift = yShift
             ...
             ,simCor_j = simCor_j
             ,simCor_jt1 = simCor_jt1
             ,simCor_jt2 = simCor_jt2
             ,best_eps_j = best_eps_j
             ,use_best_eps_j = use_best_eps_j
)
