...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SCALAR(simCor_jt1);
  DATA_SCALAR(simCor_jt2);
  DATA_VECTOR(best_eps_j);
  DATA_INTEGER(use_best_eps_j);
...
// 	  }	
  ...
  SIMULATE {
    ...
    if(re_jt==1){
      AR1(rho1_jt(0),AR1(rho2_jt(0))).simulate(s_eps_jt);
    }
    if(re_j==1){
      AR1(rho_j(0)).simulate(s_eps_j);
    }
    // AR1(rho_t(0)).simulate(s_eps_t);
    
    if(re_jt==0){ //interaction
      s_eps_jt.setZero();
    }
    if(re_j==0 & use_best_eps_j==0){
      s_eps_j.setZero();
    }
    if(re_j==0 & use_best_eps_j==1){
      s_eps_j = best_eps_j;
    }
